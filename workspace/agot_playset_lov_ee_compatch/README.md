# agot_playset_lov_ee_compatch — module state

The Essos Expanded part of the AGOT playset compatch: everything the playset
needs once Essos Expanded: The Further East is enabled. It covers the map merge,
terrain and graphical regions, the eastern government and faith history, and the
two upstream rebases that stack depends on.
...
Essos Expanded requires Legacy of Valyria, so this module is only ever enabled
on top of both others and is never the only compatch present. Load position:
last, after every parent below and after the other two. Its `ck3-tiger.conf`
loads both of them, because its map and history outputs are last-writer
decisions over `agot_playset_lov_compatch` in particular.
...
Six of its files restate a path `agot_playset_lov_compatch` also owns —
`map_data/definition.csv`, `map_data/island_region.txt`, the two mapobject
files, and the building and special-building locators. That module's copies are
the LoV map without Further East; these are the merged map, and they win
wherever this module is enabled.
...
```sh
ck3mm mod generate agot_playset_lov_ee_compatch
ck3mm mod generate agot_playset_lov_ee_compatch --apply
```
...
cache under `.ignored/cache/agot_playset_lov_ee_compatch/` absorbs the repeat
cost of the classifier, and its schema key changes whenever the merged
definition table or any parent does.
