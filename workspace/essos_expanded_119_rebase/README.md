# essos_expanded_119_rebase — module state
...
`1.18.4`, and this history uses LoV's government database. Load position:
immediately after Essos Expanded and before **Essos Expanded - TempLoV
Compatch**.
...
None. This module has no `mod.toml` and no generator; both overrides are
hand-authored. `ck3-tiger.conf` declares the dependency load order for static
validation only.

```sh
ck3mm mod validate essos_expanded_119_rebase
```

## Re-audit

Manual. Recompare both overrides after every update to Workshop mod
`3682802751`, checking that the 54 dated-capital blocks and the 410 converted
secondary baronies still match the parent.
