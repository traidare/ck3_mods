...
AGOT+ assigns perks to historical characters without first checking whether they
are alive at the selected bookmark. CK3 1.19 rejects `add_perk` on dead
characters, so every such assignment is a runtime error at game start. This
rebase gates each AGOT+ perk assignment with `is_alive = yes`. Living characters
receive exactly the same perks, while dead or not-yet-alive characters skip an
effect CK3 would reject.
...
### Missing modifier and incomplete branches
...
The `mod.toml` manifest selects AGOT+ and this rebase's destination-specific
staged generator.
