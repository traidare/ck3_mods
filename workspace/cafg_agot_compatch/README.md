...
`A Game of Thrones` (`2962333032`). Load position: after both parents.

## Ownership

Beyond the county culture/faith integrations, this layer rebases CaFG's startup
tolerance-law triggers for AGOT's culture database. AGOT does not define the
vanilla `tradition_steppe_tolerance`; the four invalid checks are omitted while
all surviving CaFG criteria are preserved. This prevents the repeated
...
faiths, effects, and script values. Generated empty exact-path overrides keep
them disabled, matching AGOT's source rather than compiling unreachable vanilla
content into the total conversion.
...
change an object but never remove it.
...
The `mod.toml` manifest declares CaFG as a portable source and limits the staged
generator to this compatch's owned outputs.

## Re-audit

The generator verifies the exact missing type and branch counts and stops if a
CaFG update changes the source assumptions. Re-audit on any CaFG or AGOT
release.
