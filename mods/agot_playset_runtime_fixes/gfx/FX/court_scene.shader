...
VertexStruct VS_OUTPUT_PDXMESHPORTRAIT
{
	...
	# MOD(agot)
	# Parity with jomini/portrait.shader: guards the decal walks against the broken
	# decal buffers of bookmark-screen style renders. Cannot use bool (OpenGL).
	uint AGOT_IsDynamicTerrainLoaded : TEXCOORD8;
	# Smuggled per-character values, searched once in the vertex shader instead of
	# per pixel (see AGOT_GatherSmuggledPortraitData in agot_portrait_decals_shared.fxh):
	# x = skin palette (MAGENTA), y = clothing decay (RED), z = salt-n-pepper hair (GREEN).
	float3 AGOT_SmuggledWeights : TEXCOORD9;
	uint AGOT_DecayDiffuseIndex : TEXCOORD10;
	# END MOD
};
	...
	MainCode VS_standard
	{
		...
		[[
			PDX_MAIN
			{
				...
				// MOD(agot) Parity with jomini/portrait.shader VS_standard.
				#ifdef AGOT_PORTRAIT_SKIN
					Out.AGOT_IsDynamicTerrainLoaded = AGOT_AreTerrainMarkerDecalsLoaded() ? 1 : 0;
				#else
					Out.AGOT_IsDynamicTerrainLoaded = 1;
				#endif // !AGOT_PORTRAIT_SKIN

				AGOT_SSmuggledPortraitData Smuggled = AGOT_GatherSmuggledPortraitData();
				Out.AGOT_SmuggledWeights = float3( Smuggled.SkinPaletteWeight, Smuggled.DecayWeight, Smuggled.HairAgingWeight );
				Out.AGOT_DecayDiffuseIndex = Smuggled.DecayDiffuseIndex;
				// END MOD
	...
	MainCode PS_skin
	{
		...
		[[
			PDX_MAIN
			{
				...
				//Skin palette blend strength, searched once in the vertex shader (MAGENTA control decal).
				float Weight = Input.AGOT_SmuggledWeights.x;
		...
		[[
			PDX_MAIN
			{
				...
				// MOD(agot)
					...
					//Clothing decay strength + mask texture, searched once in the vertex shader (RED control decal).
					float Weight = Input.AGOT_SmuggledWeights.y;
					uint DiffuseIndex = Input.AGOT_DecayDiffuseIndex;
	...
	MainCode PS_hair
	{
		...
		[[
			PDX_MAIN
			{
				...
				// MOD(agot) Hair aging, parity with jomini/portrait.shader PS_hair.
				#ifdef HAIR_AGING_DECAL

				//Get base hair texture
				float4 BaseDiffuse = PdxTex2D( DiffuseMap, UV0 );

				//Salt n' Pepper Hair strength, searched once in the vertex shader (GREEN control decal).
				float Weight = Input.AGOT_SmuggledWeights.z;
...
Effect portrait_skin
{
	...
	Defines = { "SKIN_SCATTERING" "TRANSLUCENCY" "PDX_MESH_BLENDSHAPES" "AGOT_PORTRAIT_SKIN" }
	# END MOD
}
...
Effect portrait_skin_face
{
	...
	Defines = { "SKIN_SCATTERING" "TRANSLUCENCY" "ENABLE_TEXTURE_OVERRIDE" "PDX_MESH_BLENDSHAPES" "AGOT_PORTRAIT_SKIN" }
	# END MOD
}
...
Effect portrait_skin_hair_color
{
	VertexShader = "VS_standard"
	PixelShader = "PS_skin"
	Defines = { "FAKE_SSS_EMISSIVE" "SKIN_TO_HAIR_COLOR" "PDX_MESH_BLENDSHAPES" "AGOT_PORTRAIT_SKIN"}
}
...
Effect portrait_skin_no_decal_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_court_selection"
	Defines = { "PDX_MESH_BLENDSHAPES" }
...
Effect portrait_skin_no_decalShadow
{
	VertexShader = "VertexPdxMeshStandardShadow"
	PixelShader = "PixelPdxMeshStandardShadow"
	RasterizerState = "ShadowRasterizerState"
...
Effect AGOT_snap_to_terrain
{
	VertexShader = "VS_standard"
	PixelShader = "PS_noop"
}

Effect AGOT_snap_to_terrain_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_noop"
}

Effect AGOT_snap_to_terrain_selection_mapobject
{
	VertexShader = "VS_standard"
	PixelShader = "PS_noop"
}

Effect AGOT_snap_to_terrain_mapobject
{
	VertexShader = "VS_standard"
	PixelShader = "PS_noop"
}
...
Effect portrait_eye_shift
{
	VertexShader = "VS_standard"
	PixelShader = "PS_eye"
	Defines = { "EYE_DECAL" "HSV_SHIFT" "PDX_MESH_BLENDSHAPES"}
}
...
Effect portrait_attachment_refract
{
	...
	BlendState = "hair_alpha_blend"
	#DepthStencilState = "hair_alpha_blend"
	Defines = { "USE_CHARACTER_DATA" "ATTACHEMENT_DECAY" "PDX_MESH_BLENDSHAPES" "REFRACT"}
}

Effect portrait_attachment_refract_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_court_selection"
	Defines = { "PDX_MESH_BLENDSHAPES" }
...
Effect portrait_attachment_pattern_refract
{
	...
	BlendState = "hair_alpha_blend"
	#DepthStencilState = "hair_alpha_blend"
	Defines = { "VARIATIONS_ENABLED" "USE_CHARACTER_DATA" "ATTACHEMENT_DECAY" "PDX_MESH_BLENDSHAPES" "REFRACT"}
}

Effect portrait_attachment_pattern_refract_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_court_selection"
	Defines = { "PDX_MESH_BLENDSHAPES" }
...
Effect portrait_dragon_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_court_selection"
	Defines = { "PDX_MESH_BLENDSHAPES"}
...
Effect portrait_dragon_shadow_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_noop"
}
...
Effect portrait_dragon_decals
{
	VertexShader = "VS_standard"
	PixelShader = "PS_dragon"
	Defines = { "ALPHA_TO_COVERAGE" "PDX_MESH_BLENDSHAPES" "DECALS"}
}

Effect portrait_dragon_decals_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_court_selection"
...
Effect portrait_dragon_decals_shadow_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_noop"
}
...
Effect portrait_dragon_wing_decals_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_court_selection"
	RasterizerState = "rasterizer_no_culling"
...
Effect portrait_dragon_wing_decals_shadow_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_noop"
}
...
Effect sine_flag_animation_acg_coa_selection
{
	VertexShader = "VS_standard"
	PixelShader = "PS_noop"
}

Effect sine_flag_animation_acg_coa
{
	VertexShader = "VS_standard"
	PixelShader = "PS_noop"
}
