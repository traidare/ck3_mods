...
VertexStruct VS_OUTPUT
{
	...
	# MOD(agot)
	int AGOT_TerrainVariantIndex : TEXCOORD7;
	# END MOD
};
...
VertexShader =
{
	...
	MainCode VS_standard
	{
		...
		[[
			PDX_MAIN
			{
				...
				// MOD(agot)
				AGOT_RETRIEVE_AND_FILTER_TERRAIN_VARIANT_STANDARD;
				// END MOD
				return Out;
			}
		]]
	...
	MainCode AGOT_VertexPdxMeshStandardShadow
	{
		Input = "VS_INPUT_PDXMESHSTANDARD"
		...
		[[
			PDX_MAIN
			{
				...
				int AGOT_TerrainVariantIndex;
				AGOT_RETRIEVE_AND_FILTER_TERRAIN_VARIANT_STANDARD_EXPLICIT(AGOT_TerrainVariantIndex);
...
PixelShader =
{
	...
	MainCode SKYX_PS_sky
	{
		...
		[[
			PDX_MAIN
			{
				return SKYX_GetSkyboxCubemapSample(Input.WorldSpacePos);
			}
		]]
	}
	# END MOD
	...
	MainCode PS_standard
	{
		...
		[[
			...
			{
				// MOD(agot)
				#if defined(AGOT_MAP_DEMO) && defined(SELECTION_MARKER)
					discard;
				#endif
				// END MOD
				...
				float2 ColorMapCoords =  Input.WorldSpacePos.xz *  WorldSpaceToTerrain0To1;
					...
					// MOD(agot)
					// Second, complementary snow layer (tree.shader applies both in the same
					// order): ApplySnowMaterialMesh() above blends the snow MATERIAL
					// (diffuse/normal/properties), while this call tints by winter severity.
					Diffuse.rgb = ApplyDynamicMasksDiffuse( Diffuse.rgb, Normal, ColorMapCoords, Input.AGOT_TerrainVariantIndex );
					// END MOD
				#endif

				float FogOfWarAlphaValue = PdxTex2D( FogOfWarAlpha, ColorMapCoords ).r;

				// MOD(agot) vanilla wraps this in #ifdef MAP_LIGHTING_HACK with a byte-identical multi-line variant; collapsed.
				...
				//	Color = ApplyFogOfWar( Color, Input.WorldSpacePos, FogOfWarAlpha );
					...
					Color = AGOT_ApplyAtmosphericEffects(
						Color,
						Input.WorldSpacePos,
						FogOfWarAlpha,
						AGOT_MakeAtmosphericEffectParamsGeneric(Input.AGOT_TerrainVariantIndex)
					);
				#endif
...
Effect standard_winter_mapobject
{
	VertexShader = "VS_mapobject"
	PixelShader = "PS_standard"
	Defines = { "APPLY_WINTER" "AGOT_ENABLE_DYNAMIC_TERRAIN" "MAP_LIGHTING_HACK" }
}
