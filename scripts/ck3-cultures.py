...
PLAYSETS_SCRIPT = Path(__file__).with_name("ck3-playsets.py")
...
def invoke_playsets(
    ...
    if not PLAYSETS_SCRIPT.is_file():
        fail(f"playset helper not found: {PLAYSETS_SCRIPT}")
    command = [sys.executable, str(PLAYSETS_SCRIPT), "export"]
    if playset_name:
        command.append(playset_name)
    if db_path is not None:
        command.extend(["--db", str(db_path)])

    environment = os.environ.copy()
    environment["CK3_PARADOX_DIR"] = str(paradox_dir)
    try:
        ...
        fail(f"could not invoke {PLAYSETS_SCRIPT}: {exc}")
    if result.returncode != 0:
        detail = result.stderr.strip() or result.stdout.strip()
        fail(f"{PLAYSETS_SCRIPT.name} export failed: {detail}")
    if result.stderr:
        sys.stderr.write(result.stderr)
    try:
        data = json.loads(result.stdout)
    except json.JSONDecodeError as exc:
        fail(f"{PLAYSETS_SCRIPT.name} returned invalid JSON: {exc}")
    if not isinstance(data, dict):
        fail(f"{PLAYSETS_SCRIPT.name} returned a non-object JSON value")
    return data
...
def required_directory(argument: Path | None, environment_name: str) -> Path:
    ...
    if raw is None or str(raw).strip() == "":
        fail(
            f"pass --{environment_name[4:].lower().replace('_', '-')} or set ${environment_name}"
        )
...
def load_database(args: argparse.Namespace) -> Database:
    game_dir = required_directory(getattr(args, "game_dir", None), "CK3_GAME_DIR")
    workshop_dir = required_directory(
        getattr(args, "workshop_dir", None), "CK3_WORKSHOP_DIR"
    )
    paradox_dir = required_directory(
        getattr(args, "paradox_dir", None), "CK3_PARADOX_DIR"
    )
    playset = invoke_playsets(
        getattr(args, "playset", None),
        getattr(args, "db", None),
        paradox_dir,
    )
    playset_name = str(playset.get("name") or "").strip()
    if not playset_name:
        fail(f"{PLAYSETS_SCRIPT.name} returned a playset without a name")
...
def common_options() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=argparse.SUPPRESS,
        help="Launcher database override forwarded to ck3-playsets.py",
    )
    parser.add_argument(
        "--format",
        dest="output_format",
