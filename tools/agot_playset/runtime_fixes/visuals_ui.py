...
# `00_default_cultures.txt` holds the generic court scenes. Additional Models
# owns it, and the Additional Models/LoV compatch replaces the whole file, so
# the guarded copy is rebased from whichever of them is effective in a profile.
GENERIC_SCENE_SOURCES = {
    "3319354609": (10, "Additional Models generic court scenes"),
    "3773616784": (12, "Additional Models/LoV generic court scenes"),
}


def generate_scene_culture_owner_guards(
    inputs: RunInputs,
    *,
    generic_source: str = "3773616784",
    include_agot: bool = True,
) -> None:
    generic_expected, generic_label = GENERIC_SCENE_SOURCES[generic_source]
    sources = [
        (
            generic_source,
    ...
    for workshop_id, relative, expected, label in sources:
        # Additional Models ships this file with CRLF endings; the guard rewrites
        # trigger bodies line by line, so normalize before editing them.
        text = read_text(inputs.WORKSHOP / workshop_id / relative).replace("\r\n", "\n")
