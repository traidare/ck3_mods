...
def generate_tour_general_events(
    inputs: RunInputs, *, workshop_id: str = "3788296332"
) -> None:
    """Order the tour-general 3001 stop-host guard before its scope switch.

    AGOT and the LoV bridge each ship a whole-file copy of the path with the
    same trigger ordering, so the repair restates whichever copy is the
    effective last writer in the profile and carries no other AGOT file.
    """
...
""",
        expected=1,
        label="tour-general 3001 stop-host guard ordering",
    )
    write_text(inputs.OUTPUT, relative, text)


def generate_agot_azor_tour_events(inputs: RunInputs) -> None:
    """Guard the AGOT Azor Ahai tour callbacks independently of LoV."""
