...
def generate_title_on_action_repairs(
    inputs: RunInputs, *, workshop_id: str = "3788296332"
) -> None:
    """Repair title on-actions: yurt setup and noble-family title churn.

    AGOT and the LoV bridge each ship a whole-file copy of the path and both
    carry the same nomad setup and `create_noble_family_effect` call sites, so
    the restated file is taken from whichever copy is the effective last writer
    in the profile.
    """
    relative = "common/on_action/title_on_actions.txt"
    source = read_text(inputs.WORKSHOP / workshop_id / relative)
    ...
    source = replace_exact(
        source, old_1100, new_1100, expected=1, label="nomad 1100 yurt setup"
    )
    old_900 = (
        "\t\t\t\ttitle_domicile = {\n"
        "\t\t\t\t\tadd_domicile_building = yurt_main_02\n"
    ...
    source = replace_exact(
        source, old_900, new_900, expected=1, label="nomad 900 yurt setup"
    )
    ...
    source = replace_exact(
        ...
        "\t\t\t}\n",
        expected=1,
        label="noble-family creation deferral (top-liege vassal)",
    )
    source = replace_exact(
        source,
        "\t\t\t\t\tcreate_noble_family_effect = { GOVERNMENT_GIVER = this }\n"
        ...
        "\t\t\t\t\t}\n",
        expected=1,
        label="noble-family creation deferral (independent ruler)",
    )
...
def generate_agot_title_on_action_septon_naming(
    inputs: RunInputs, *, workshop_id: str = "3788296332"
) -> None:
    ...
    `give_nickname` is idempotent. AGOT and LoV's AGOT bridge each ship a
    whole-file copy of the path and neither grants the nickname, so the restated
    file is taken from whichever copy is the effective last writer in the
    profile; every other on-action in it is kept.
    """
