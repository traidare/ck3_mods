...
def guard_scene_culture_triggers(text: str, *, expected: int, label: str) -> str:
    ...
    # Parents indent the scene's own `trigger` with either a tab or spaces, and
    # a single file mixes both, so the opening and closing lines are matched by
    # shape rather than by one literal.
    opening = re.compile(r"(?m)^[ \t]+trigger\s*=\s*\{[ \t]*\n")
    closing = re.compile(r"(?m)^[ \t]+\}\n\}\Z")
    for key in keys:
        block = extract_top_level_block(text, key)
        opened = opening.search(block)
        closed = closing.search(block)
        if opened is None or closed is None:
            raise RuntimeError(f"{label}: unexpected trigger structure for {key}")
        body_start = opened.end()
        body_end = closed.start()
        if body_end <= body_start:
            raise RuntimeError(f"{label}: empty trigger body for {key}")
