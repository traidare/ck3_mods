...
class MergedMap:
    """The merged province table, handed to the world stage in memory.

    Both forms travel together because the world stage needs each: the parsed
    rows to classify provinces, and the exact text it would otherwise have read
    back off disk to key its feature cache.
    """

    definition: str
    provinces: tuple[Definition, ...]
...
# The pins above state the baseline of the Further East stack, whose
# `special_building_locators` is merged from EEP and the Essos compatch.  With
# Legacy of Valyria alone the effective source is Seasons of Valyria, which
# places 4945 a fraction away from that merge.  COW's mesh needs the same final
# placement either way, so only the overridden baseline is profile-specific and
# the pinned values stay shared.
LOV_LOCATOR_BASELINES: dict[tuple[str, int], dict[str, str]] = {
    (SPECIAL_BUILDING_LOCATORS, 4945): {
...
def apply_locator_pins(
    ...
    merged: dict[int, str],
    relative: str,
    baselines: dict[tuple[str, int], dict[str, str]] | None = None,
) -> tuple[list[int], dict[int, str]]:
    """Override merged locator records field by field with their pinned values.

    `baselines` replaces individual expected-upstream fields for a profile whose
    effective source differs from the Further East stack the pins are stated
    against.  It never changes what is written, only what must still be true of
    the input for the pin to remain a reviewed decision.
    """
    pins = {
    ...
    overrides = {
        key: fields
        for (path, key), fields in (baselines or {}).items()
        if path == relative
    }
    unknown = set(overrides) - set(pins)
    if unknown:
        raise RuntimeError(
            f"locator baseline override for {relative} names unpinned records: "
            f"{sorted(unknown)}"
        )
    result = dict(merged)
    for key, (title, baseline, pinned) in pins.items():
        baseline = baseline | overrides.get(key, {})
...
class ProvinceGeometry:
    ...
    Further East's `provinces.png` is the effective raster and this layer
    supplies `definition.csv`, so the two must be read as a pair.  That pairing
    is exact rather than approximate: the thirteen NOW colour edits are a closed
    ...
    intended.  `raster` carries that raster and `id_raster` resolves it against
    `definition.csv`, so the locator placement below describes the map CK3
    loads.
    """
...
LANDED_TITLE_STACK = ("AGOT", "NOW", "LOV", "RC", "EE", "EEP", "BRIDGE")


def effective_landed_title_files(inputs: RunInputs) -> dict[str, Winner]:
    """Resolve the effective landed-title file set by relative path."""
    return file_winners(
        (Layer.of(label, inputs[label]) for label in LANDED_TITLE_STACK),
        LANDED_TITLES,
        recursive=True,
    )
...
def effective_title_keys(title_files: dict[str, Winner]) -> frozenset[str]:
    """Return every title key the effective landed-title stack defines."""
    keys: set[str] = set()
    for winner in title_files.values():
        uncommented = "\n".join(
            line.split("#", 1)[0] for line in winner.text().splitlines()
        )
...
def effective_titled_provinces(
    ...
    for relative, winner in sorted(title_files.items()):
        for province in province_ids_from_landed_titles(winner.text()):
...
def quarantined_default_map(
    inputs: RunInputs, definition: str, geometry: ProvinceGeometry
) -> tuple[str, dict[str, object]]:
    """Make every reviewed, titleless definition non-passable."""
    default_map = read_text(inputs.current_map_source(DEFAULT_MAP))
...
def assert_compact_provinces(id_raster, colours: dict[int, int]) -> None:
    ...
    A province whose pixels reach far beyond its own bounds is either one of the
    reviewed zones that genuinely does, or stray pixels of one province left
    behind inside another - a leak that hands two provinces on opposite sides of
    the map a shared border, which CK3 then reads as adjacency.
    """
    import numpy as np
...
def province_geometry(inputs: RunInputs, definition_text: str) -> ProvinceGeometry:
    ...
    return ProvinceGeometry(
        land=land_provinces(read_text(inputs.current_map_source(DEFAULT_MAP)), defined),
        anchor=anchor,
        id_raster=id_raster,
        raster=rgb,
        width=width,
...
def adopt_locator_file(
    ...
    text = read_text(inputs["BRIDGE"] / relative)
...
def stacked_overlays(inputs: RunInputs, relative: str, parse) -> tuple[Overlay, ...]:
    """NOW diffs against AGOT."""
    agot = parse(read_text(inputs["AGOT"] / relative))
    now = parse(read_text(inputs["NOW"] / relative))
...
def merge_locator_file(
    ...
    text = read_text(inputs.current_map_source(relative))
    ...
    if relative == BUILDING_LOCATORS:
        ...
        current = replace_locator_band(
            current,
            locator_records(read_text(inputs["AGOT"] / relative)),
...
def merge_object_file(inputs: RunInputs, relative: str) -> str:
    current = split_objects(read_text(inputs.current_map_source(relative)))
    return merge_file_blocks(
        current,
...
def merge_definition(inputs: RunInputs) -> str:
    lines, agot = definition_lines(read_text(inputs["AGOT"] / DEFINITION))
    _, now = definition_lines(read_text(inputs["NOW"] / DEFINITION))
    eep_lines, eep = definition_lines(read_text(inputs["EEP"] / DEFINITION))
    if len(eep) != DEFINITION_ROWS or max(eep) != DEFINITION_ROWS - 1:
        raise RuntimeError(
    ...
    for relative in LOCATOR_FILES:
        _, _, _, agot_locators = locator_records(read_text(inputs["AGOT"] / relative))
        _, _, _, now_locators = locator_records(read_text(inputs["NOW"] / relative))
        required.update(
...
def assert_restored_regions_resolve(
    inputs: RunInputs, restored: dict[str, str], definition: str
) -> None:
    ...
    _, agot_rows = definition_lines(read_text(inputs["AGOT"] / DEFINITION))
    _, shipped_rows = definition_lines(definition)
...
def parent_versions(inputs: RunInputs) -> dict[str, str]:
    ...
    modules = {
        ...
        "NOW": inputs["NOW"],
        "LoV": inputs["LOV"],
        "LoV AGOT bridge": inputs["RC"],
        "EEP": inputs["EEP"],
        "EEC": inputs["BRIDGE"],
    }
    return {
        label: re.search(
            r'(?m)^version="([^\"]+)"', read_text(root / "descriptor.mod")
        ).group(1)
        for label, root in modules.items()
    }


def merge(inputs: RunInputs) -> MergedMap:
    ...
    return MergedMap(
        ...
        provinces=tuple(
            parse_definitions(definition, label="merged", expected_rows=DEFINITION_ROWS)
        ),
    )
