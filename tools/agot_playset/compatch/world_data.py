...
# CK3 reads a geographical-region directory's own files before its
# subdirectories, so NOW's `replace/` survey is parsed after every root file in
# it and is the last definition of each key it names. This output sits in that
# same subdirectory and sorts after it, which is what makes this module's
# merged and generated blocks the ones the game keeps.
REGION_DIR = "map_data/geographical_regions"
REGION_LAST_WRITER_OUTPUT = f"{REGION_DIR}/replace/zzzz_agot_now_lov_ee_regions.txt"
...
def feature_source_state(
    ...
    *,
    ...
    return {
        "schema_version": 3,
        # The merged province table is produced in memory by the map stage, so it
        # is keyed by its content rather than by a path the cache could stat.
        "merged_definition": sha256_bytes(merged_definition.encode("utf-8")),
        "expected": {
...
# The parent stack this stage resolves terrain and geographical regions through,
# in load order. The Essos Expanded compatch is absent deliberately: it ships
# neither province terrain nor geographical regions.
WORLD_STACK = ("AGOT", "NOW", "LOV", "RC", "EE", "EEP")
...
class WorldDataPipeline:
    ...
    def stack(self) -> list[Layer]:
        return [Layer.of(label, self.inputs[label]) for label in WORLD_STACK]
    ...
    def load_sources(self) -> None:
        ...
        self.workshop = self.inputs.workshop
        ...
        assets = self.inputs.assets / "world_data"
        ...
        self.reference_paths = self.inputs.references
        ...
        merged_definitions = self.merged.provinces
        self.eep_definitions = parse_definitions(
            read_text(self.workshop["EEP"] / "map_data/definition.csv"),
            label="Further East definition.csv",
    ...
    def build_features(self) -> None:
        ...
        self.features = load_or_compute_features(
            cache_dir=self.root / ".ignored/cache/agot_playset_lov_ee_compatch",
            cache_key=cache_key,
        ...
        # the training labels.
        terrain_by_province = key_winners(
            terrain_modules, "common/province_terrain", parse_scalar_terrain
        )
        ...
        self.valid_terrains = set(
            key_winners(
                terrain_modules, "common/terrain_types", parse_terrain_type_keys
            )
        )
        gameplay_terrains = {
    ...
    def build_graphical_regions(self) -> None:
        ...
        self.region_blocks = key_winners(
            self.stack(),
            "map_data/geographical_regions",
    ...
    def build_region_last_writer(self, emitted: dict[str, str]) -> bytes:
        """Restate this module's definition of the regions NOW's survey redefines.

        NOW ships its Westeros survey under `replace/`, which CK3 parses after
        both region files this module writes into the directory root. Every key
        that survey names would otherwise take NOW's block at load, discarding
        the merged coastal territory and the generated graphical styles this
        run computes. Restating them here, from the same subdirectory under a
        later filename, is what makes them effective.

        Each key takes this run's graphical block where the run emits one, and
        the merged block from the map-merge stage otherwise.
        """
        ...
        if unshipped:
            raise AssertionError(
                "NOW's replace/ survey names regions this module does not ship: "
                f"{sorted(unshipped)}; carry them in the merge before this file "
                "can decide them"
            )
        self.assert_regions_parsed_last(set(merged) | set(emitted))
        lines = [
            "\ufeff# Generated by ck3mm from the mod's workspace implementation.",
            "# Last definition of every region NOW's replace/ survey redefines.",
            "",
        ]
        for key in now_order:
            lines.append((emitted.get(key) or merged[key]).strip("\n"))
            lines.append("")
        return ("\n".join(lines)).encode("utf-8")

    def assert_regions_parsed_last(self, shipped: set[str]) -> None:
        """Fail when a parent's region file is parsed after this module's.

        CK3 reads a directory's own files before its subdirectories, so a file's
        position is its subdirectory path first and its filename second. A
        parent file that sorts after the output above and names a region this
        module ships decides that region instead, which is silent: the block is
        still generated, still shipped, and simply not the one the game keeps.
        """
        output = Path(REGION_LAST_WRITER_OUTPUT).relative_to(REGION_DIR)
        last = (output.parts[:-1], output.name)
        overriding = []
        for label in WORLD_STACK:
            base = self.inputs[label] / REGION_DIR
            if not base.is_dir():
                continue
            for path in sorted(base.rglob("*.txt")):
                relative = path.relative_to(base)
                if (relative.parts[:-1], relative.name) <= last:
                    continue
                keys = set(
                    top_level_blocks(
                        read_text(path),
                ...
                if contested := keys & shipped:
                    overriding.append(f"{label}:{relative} {sorted(contested)[:5]}")
        if overriding:
            raise AssertionError(
                "parent region files are parsed after this module's: "
                + "; ".join(overriding)
                + "; this module's blocks no longer decide those regions"
            )
    ...
    def write_outputs(self) -> None:
        ...
        outputs = {
            ...
            self.module / REGION_LAST_WRITER_OUTPUT: self.build_region_last_writer(
                emitted
            ),
        }
...
def build(inputs: RunInputs, merged: MergedMap) -> None:
    WorldDataPipeline(inputs, merged).run()
